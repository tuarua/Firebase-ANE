    ...
    func startTrace(ctx: FREContext, argc: FREArgc, argv: FREArgv) -> FREObject? {
        guard argc > 0,
            let name = String(argv[0])
                ...
                return FreArgError(message: "startTrace").getError(#file, #line, #column)
        }
        if traces[name] == nil {
    ...
    func stopTrace(ctx: FREContext, argc: FREArgc, argv: FREArgv) -> FREObject? {
        guard argc > 0,
            let name = String(argv[0])
                ...
                return FreArgError(message: "stopTrace").getError(#file, #line, #column)
        }
        traces[name]?.stop()
        return nil
    }
    ...
    func setIsDataCollectionEnabled(ctx: FREContext, argc: FREArgc, argv: FREArgv) -> FREObject? {
        guard argc > 0,
            let value = Bool(argv[0])
                ...
                return FreArgError(message: "setIsDataCollectionEnabled").getError(#file, #line, #column)
        }
    ...
    func setIsInstrumentationEnabled(ctx: FREContext, argc: FREArgc, argv: FREArgv) -> FREObject? {
        guard argc > 0,
            let value = Bool(argv[0])
                ...
                return FreArgError(message: "setIsInstrumentationEnabled").getError(#file, #line, #column)
        }
